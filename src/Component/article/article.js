...
function CommentList({ comments, deleteComment }) {
  if(!isEmpty(comments)){
    return comments.map((item) => (
      <div className="row comments-item" key={item.id}>
        <div className="col-md-1 text-align-center">
          <div className="comments-image">
            <img alt="" src={item.author.image}/>
          </div>
          <Link to={`/profiles/${item.author.username}`}>{item.author.username}</Link>
        </div>
        <div className="col-md-10 comments-body">
          <div>{item.body}</div>
          ...
          <div className="row comments-item">
            ...
            <div className="col-md-10 comments-body">
              <form onSubmit={this.handleSubmit}>
                <input className="form-control" type="text" placeholder="What do you want to say?" value={this.state.newComment} onChange={this.handleChange}/>
              </form>
            </div>
          </div>
